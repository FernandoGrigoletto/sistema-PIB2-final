...
import {
  ...
  FaUserPlus,
  FaQuestionCircle,
  FaUser, // Adicionado do segundo código para o link de Membros
} from "react-icons/fa";
...
const Sidebar = () => {
  ...
  // MELHORIA: Estilização dinâmica mais robusta com Bootstrap (Padrão Código 1)
  const getNavLinkClass = ({ isActive }) => {
    const baseClasses =
      "nav-link d-flex align-items-center gap-3 py-3 px-4 transition-all";
    const activeClasses =
      "bg-primary bg-opacity-10 text-primary fw-bold border-end border-4 border-primary";
  ...
  return (
    <div
      className="d-flex flex-column flex-shrink-0 bg-white sidebar-container shadow-sm h-100"
      style={{ width: "280px" }}
    >
      {/* Cabeçalho com Logo */}
      ...
      <Nav className="flex-column mb-auto mt-3 gap-1">
        ...
        {/* Integração do Segundo Código: Link para Membros */}
        <Nav.Item>
          <NavLink to="/membros" className={getNavLinkClass}>
            <FaUser size={18} /> Membros
          </NavLink>
        </Nav.Item>
        ...
        {isAdmin && (
          <>
            <div
              className="text-uppercase text-muted fw-bold small px-4 mt-4 mb-2"
